...
function User (props)  {
    ...
    const init=async()=>{
        await fetch("https://web-production-a96f.up.railway.app/test_app/"+userID)
            .then(response => {
                if (!response.ok) {
    ...
    async function handleDelete(){
        try{        
            await fetch("https://web-production-a96f.up.railway.app/test_app/"+user.my_id+"/", {method: "DELETE",headers: { "content-type": "application/json", }})
            .then((response)=>{
    ...
    const put=async (comment)=>{
        await fetch("https://web-production-a96f.up.railway.app/test_app/"+user.my_id+"/",{method:"PATCH", body:JSON.stringify({ username: comment }),headers: {"Content-Type": "application/json",}})
          .then(response => {
              if (!response.ok) {
              throw new Error('Network response was not ok');
              }
